...
meta <- meta |>
  mutate(is_caribbean = country %in% caribbean_countries,
...
p_serotype <- carib |>
  ...
  geom_col(width = 0.7) +
  scale_fill_manual(
    values = c("DENV-1" = "#E63946", "DENV-2" = "#457B9D",
...
ggsave("results/figures/serotype_composition.png",
       p_serotype, width = 9, height = 6, dpi = 300)
...
ggsave("results/figures/year_serotype_heatmap.png",
       p_heat, width = 10, height = 4, dpi = 300)
...
ggsave("results/figures/jamaica_timeline.png",
       p_timeline, width = 10, height = 5, dpi = 300)

# ---------------------------------------------------------------------------
# 6. Lineage summary table
# ---------------------------------------------------------------------------
lineage_summary <- carib |>
  count(serotype, lineage, sort = TRUE) |>
  group_by(serotype) |>
